...
class NativeByteBufferTest {
    ...
    @Test
    fun readAny() {
        val buf = byteBufferFrom(refRead.copyOf(), Endianness.LITTLE_ENDIAN).toNativeByteBuffer()
    ...
    fun testToMutableNativeByteBuffer1() {
        assertContentEquals(byteBufferFrom(refRead.copyOf()).toNativeByteBuffer().toMutableNativeByteBuffer().toMutableByteBuffer().getArray(), refRead)
    }

    @Test
    fun copyInto() {
        val buf = mutableByteBufferWith(128)
        byteBufferFrom(refRead.copyOf()).toNativeByteBuffer().copyInto(buf, 0..128)
